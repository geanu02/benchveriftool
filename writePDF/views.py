...
def home_view(request, *args, **kwargs):
    ...
    c_3 = pd.DataFrame()
    if useGoogle:
        c_1 = clean_s1_df2(gData(sheet_name, 1))
        c_2 = clean_s1_df2(gData(sheet_name, 1))
        c_3 = clean_s2_df1(gData(sheet_name, 2))
    else:
...
def mock_view(request, *args, **kwargs):
    ...
    if request.method == "POST":
        ...
        peerGrp_index = request.POST.get('peerGrp_id')
        output_file = pdf_production(stat=stat_dict,
                                     elem=elem_dict(c_num=int(client_index),
                                                    p_num=int(peerGrp_index),
                                                    c_1=c_1,
                                                    c_2=c_2,
                                                    c_3=c_3,
                                     ...
                                     c_num=int(client_index),
                                     mock=True)
    return render(request, "mock.html", con)


def load_stat():
    if settings.DEBUG == True:  # on Local Machine
        # if settings.DEBUG == False: on PythonAnywhere
        output_url = os.path.join(
            settings.BASE_DIR, "writePDF/static/writePDF/data/output_doc.pdf")
    else:
        output_url = staticfiles_storage.path("writePDF/data/output_doc.pdf")
    return {
        "df1":
        staticfiles_storage.path("writePDF/data/verifAll.csv"),
        "df2":
        staticfiles_storage.path("writePDF/data/verifP4.csv"),
        "headImage":
        staticfiles_storage.path("writePDF/images/head.png"),
        "output":
        output_url,
        "elemHeadtxt":
...
def load_stat_mock():
    # head_url = "writePDF/images/head.png"
    if settings.DEBUG == True:  # on Local Machine
        # if settings.DEBUG == False: on PythonAnywhere
        output_url = os.path.join(
            settings.BASE_DIR, "writePDF/static/writePDF/mock/output_doc.pdf")
    else:
        output_url = staticfiles_storage.path("writePDF/mock/output_doc.pdf")
    return {
        "df1":
        staticfiles_storage.path("writePDF/mock/mockAll.csv"),
        "df2":
        staticfiles_storage.path("writePDF/mock/mockP4.csv"),
        "headImage":
        staticfiles_storage.path("writePDF/mock/head_mock.png"),
        "output":
        output_url,
        "elemHeadtxt":
        staticfiles_storage.path("writePDF/mock/element_head.txt"),
        "elemCtxt":
        staticfiles_storage.path("writePDF/mock/element_c.txt"),
        "elemCbullettxt":
        staticfiles_storage.path("writePDF/mock/element_c_bullet.txt"),
        "elemCAtxt":
        staticfiles_storage.path("writePDF/mock/element_c_a.txt"),
        "elemCBtxt":
        staticfiles_storage.path("writePDF/mock/element_c_b.txt"),
        "elemDtxt":
        staticfiles_storage.path("writePDF/mock/element_d.txt"),
        "elemDAtxt":
        staticfiles_storage.path("writePDF/mock/element_d_a.txt"),
        "elemEtxt":
        staticfiles_storage.path("writePDF/mock/element_e.txt"),
        "elemFtxt":
        staticfiles_storage.path("writePDF/mock/element_f.txt"),
    }
...
def elem_dict(c_num, p_num, c_1, c_2, c_3, mocking):
    mock = mocking
    ...
    def data_elmB(c_num, p_list):
        ...
        df_av = elmB_process(c_1.loc[:,
                                     elmB_head].replace(refeLMHD).astype(int))
        df_cl = c_1.loc[c_num, elmB_head].replace(refeLMHD).to_list()
        return [
            ['P1', 'P2', 'P3', 'P4', 'P5', 'P6', 'P7', 'P8'],
            (
                c_name(c_num),
                [
                    [int(s) for s in df_cl],
                    df_pg,
                    df_av,
                    # Ignore list below
                    [0, 4, 0, 4, 0, 4, 0, 4]
...
def pdf_production(stat, elem, c_num, mock):
    pdf = PDF()
    pdf.set_title('Practice Verification Client Benchmark Report')
    pdf.head(stat["headImage"], c_num, elem["formatName"], stat["elemHeadtxt"],
             mock)
    ...
    pdf.element_f(elem["formatName"], elem["formatpName"], stat["elemFtxt"],
                  elem["elmF"])
    pdf.set_author("Gian Carlo Torres")
